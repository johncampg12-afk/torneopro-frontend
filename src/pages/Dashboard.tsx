  ...
  return (
    <div className="animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-4 mb-6 md:mb-8">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold">Mis Torneos</h2>
          <p className="text-slate-400 text-sm md:text-base">Gestiona todos tus campeonatos</p>
        </div>
        <Link to="/tournaments/create" className="btn-primary text-sm md:text-base">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 mb-6 md:mb-8">
        {[
          ...
          <div key={s.label} className="glass p-3 md:p-5 text-center">
            <i className={`fas ${s.icon} ${s.color} text-xl md:text-2xl mb-1 md:mb-2`}></i>
      ...
      <div className="flex flex-col sm:flex-row gap-2 md:gap-3 mb-4 md:mb-6">
        <div className="flex gap-1 md:gap-2 overflow-x-auto pb-1 md:pb-2">
          {[{id:'all',label:'Todos'},{id:'active',label:'En curso'},{id:'finished',label:'Finalizados'},{id:'draft',label:'Borradores'}].map(f => (
            <button key={f.id} onClick={() => setFilter(f.id)}
              className={`px-3 md:px-4 py-2 rounded-xl text-xs md:text-sm font-medium transition-all whitespace-nowrap ${filter === f.id ? 'bg-primary-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700'}`}>
              {f.label}
            </button>
          ))}
        </div>
        <div className="flex-1">
          <div className="relative">
            <i className="fas fa-search absolute left-3 md:left-4 top-1/2 -translate-y-1/2 text-slate-500 text-sm"></i>
            <input type="text" value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Buscar torneo..."
              className="input-dark pl-9 md:pl-11 text-sm" />
          </div>
        </div>
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
          {[1,2,3].map(i => <div key={i} className="glass p-4 md:p-6 h-40 md:h-48 animate-pulse bg-slate-800/50"></div>)}
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-12 md:py-20 glass rounded-2xl">
          <i className="fas fa-trophy text-3xl md:text-4xl text-slate-700 mb-3 md:mb-4"></i>
          <h3 className="text-lg md:text-xl font-bold text-slate-400 mb-2">No hay torneos {filter !== 'all' ? 'en esta categoría' : ''}</h3>
          <p className="text-slate-600 text-sm md:text-base mb-4 md:mb-6">Crea tu primer torneo y empieza a gestionar</p>
          <Link to="/tournaments/create" className="btn-primary text-sm md:text-base"><i className="fas fa-plus"></i> Crear Torneo</Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
          {filtered.map(t => {
            ...
            return (
              <div key={t.id} className="glass p-4 md:p-6 hover-lift group relative">
                <div className="flex items-start justify-between mb-3 md:mb-4">
                  ...
                  <div className="flex gap-1 md:gap-2">
                    <button onClick={() => handleCopyLink(t.shareCode)}
                      className="w-7 h-7 md:w-8 md:h-8 rounded-lg hover:bg-primary-500/20 hover:text-primary-400 flex items-center justify-center transition-colors text-slate-400"
                      title="Copiar link público">
                      <i className="fas fa-link text-xs md:text-sm"></i>
                    </button>
                    <button onClick={() => handleDelete(t.id)}
                      className="w-7 h-7 md:w-8 md:h-8 rounded-lg hover:bg-red-500/20 hover:text-red-400 flex items-center justify-center transition-colors text-slate-400">
                ...
                <Link to={`/tournaments/${t.id}`} className="block">
                  ...
                  <p className="text-slate-500 text-xs md:text-sm mb-3 md:mb-4">{getSportName(t.sport)} · {getFormatName(t.format)}</p>
                  ...
                  <div className="w-full bg-slate-800 rounded-full h-1.5 md:h-2 mb-1.5 md:mb-2">
                    <div className="bg-gradient-to-r from-primary-500 to-accent-500 h-1.5 md:h-2 rounded-full transition-all" style={{width: `${progress}%`}}></div>
                  </div>
