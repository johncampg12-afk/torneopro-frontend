        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <Link to="/" className="flex items-center gap-3">
              ...
              <span className="font-bold text-xl tracking-tight">Torneo<span className="gradient-text">Pro</span></span>
            </Link>
        ...
        <div className="glass rounded-2xl p-6 mb-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-slate-400">{getSportName(tournament.sport)} · {getFormatName(tournament.format)} · Organizado por {tournament.owner?.name || 'Anónimo'}</p>
              {tournament.location && <p className="text-slate-500 text-sm mt-1"><i className="fas fa-map-marker-alt mr-1"></i>{tournament.location}</p>}
            </div>
              ...
              <div className="w-16 h-16 rounded-full bg-slate-800 flex items-center justify-center relative">
                <svg className="w-16 h-16 transform -rotate-90">
                  ...
                  <circle cx="32" cy="32" r="28" stroke="#3b82f6" strokeWidth="4" fill="none"
                    ...
                    strokeDashoffset={`${2 * Math.PI * 28 * (1 - Math.round((playedMatches / Math.max(1, totalMatches)) * 100) / 100)}`} />
                </svg>
                <span className="absolute text-sm font-bold">{Math.round((playedMatches / Math.max(1, totalMatches)) * 100)}%</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <div key={round.id} className="glass rounded-2xl p-6">
                ...
                <div className="grid md:grid-cols-2 gap-3">
                  {round.matches.map((match: any) => {
                    ...
                    return (
                      <div key={match.id} className={`bg-slate-800/50 rounded-xl p-4 border ${match.played ? 'border-primary-500/30' : 'border-slate-800'}`}>
                        <div className="flex items-center justify-between">
