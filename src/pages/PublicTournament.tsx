...
export default function PublicTournament() {
  ...
  const playedMatches = tournament.rounds.reduce((a, r) => a + r.matches.filter(m => m.played).length, 0);
  ...
  return (
    <div className="min-h-screen bg-dark-950 animate-fade-in">
      {/* Header público con logo y nombre actualizados */}
      <div className="border-b border-slate-800/50">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-3 md:py-4">
          <div className="flex items-center justify-between">
            <Link to="/" className="flex items-center gap-3">
              <img src="/torneo-trend-sport.png" alt="Torneos TrendSport" className="w-9 h-9 rounded-lg object-contain" />
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="w-14 h-14 md:w-16 md:h-16 rounded-full bg-slate-800 flex items-center justify-center relative">
                <svg className="w-14 h-14 md:w-16 md:h-16 transform -rotate-90">
                  ...
                  <circle cx="50%" cy="50%" r="26" stroke="#3b82f6" strokeWidth="4" fill="none"
              ...
              <div key={round.id} className="glass rounded-2xl p-3 md:p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {round.matches.map((match: any) => {
                    ...
                    return (
                      <div key={match.id} className={`bg-slate-800/50 rounded-xl p-2.5 md:p-4 border ${match.played ? 'border-primary-500/30' : 'border-slate-800'}`}>
                    ...
                    <tbody>
                      {topScorers.map(p => (
                        <tr key={p.id} className="border-b border-slate-800/50">
                          ...
                          <td className="text-center py-2 flex justify-center items-center gap-2"><span className="w-2 h-2 rounded-full" style={{ backgroundColor: p.teamColor }}></span>{p.team}</td>
